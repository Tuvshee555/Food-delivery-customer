...
export default function PaymentPendingPage() {
  ...
  const hasRequestedInvoice = useRef(false); // 🔥 Prevent double request
  ...
  // ---------------------------------------
  // 2️⃣ Create invoice ONCE — no double calls
  // ---------------------------------------
  useEffect(() => {
    if (!orderId || !order?.totalPrice) return;

    if (hasRequestedInvoice.current) return; // 🔥 Stop duplicate request
